  ...
  <section class="pkmn_card_dialog">
    ...
    <div class="pkmn_card_dialog_content">
      <section class="dialog_inner">
        <section>
      <img src="${getImg(index + 1)}" class="pkmn_img_dialog" alt="Picture of the Pokemon">
        <div class="arrows">
      ...
      <div class="types">
      ${types
        .map((type) => `<span class="${type}">${FirstLetter(type)}</span>`)
        .join(" ")}
      </div>
            <p class="stat">
              HP: ${stats.find((status) => status.stat.name === "hp").base_stat}
            </p>
            <p class="stat">
            Attack: ${stats.find((status) => status.stat.name === "attack").base_stat}
          </p>
          <p class="stat">
          Defense: ${stats.find((status) => status.stat.name === "attack").base_stat}
        </p>
        </div>
        </section>
